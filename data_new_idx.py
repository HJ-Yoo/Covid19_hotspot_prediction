...
def data_loader_new_idx(args):
    data_filename = os.path.join('d{}-{}-{}_s{}'.format(args.train_days, args.delay_days, args.test_days,args.lstm_sequence_length))
    
    if not os.path.exists(os.path.join(args.data_path + 'train_'+ data_filename + '_new_idx')):
        # Dataset import 
...
#         edge_infect = np.array([edge_daegu_infect[:,0]+edge_daegu_infect[:,1], edge_daegu_infect[:,2], edge_daegu_infect[:,3], edge_daegu_infect[:,4]]).transpose()
        ...
        # sty infect (label)
        target_sty_infect = node_daegu_infect.reshape([-1,142])
        train_target_sty_infect = target_sty_infect[24*(LSTM_SEQUENCE_LENGTH + DELAY_DAYS) : 24*(LSTM_SEQUENCE_LENGTH + DELAY_DAYS + TRAIN_DAYS)]
        test_target_sty_infect = target_sty_infect[24*(LSTM_SEQUENCE_LENGTH + DELAY_DAYS + TRAIN_DAYS):]
        # mv infect (label)
        raw_mv_infect = pd.DataFrame(edge_infect, columns=['ymd_time', 'from_dong', 'to_dong', 'mv_num'])
        mv_infect = raw_mv_infect.loc[1:]
        to_dong_grouped_infect = mv_infect.groupby(['ymd_time', 'to_dong']).sum()['mv_num']
        to_dong_grouped_infect = to_dong_grouped_infect.reset_index()
        
        target_mv_infect = torch.zeros([696, 142])
        for i, ymd_time in tqdm(enumerate(ymd_time_lst)):
            for j, dong in enumerate(dong_lst):
                if not to_dong_grouped_infect[(to_dong_grouped_infect['ymd_time']==ymd_time) & (to_dong_grouped_infect['to_dong']==dong)].empty:
                    target_mv_infect[i][j] = to_dong_grouped_infect[(to_dong_grouped_infect['ymd_time']==ymd_time) & (to_dong_grouped_infect['to_dong']==dong)]['mv_num'].item()
        train_target_mv_infect = target_mv_infect[24*(LSTM_SEQUENCE_LENGTH + DELAY_DAYS) : 24*(LSTM_SEQUENCE_LENGTH + DELAY_DAYS + TRAIN_DAYS)]
        test_target_mv_infect = target_mv_infect[24*(LSTM_SEQUENCE_LENGTH + DELAY_DAYS + TRAIN_DAYS):]
                    
        train_target_infect = train_target_mv_infect + train_target_sty_infect
        test_target_infect = test_target_mv_infect + test_target_sty_infect
        
        # sty_user(label)
        target_sty_user = node_daegu_user.reshape([-1,142])
        train_target_sty_user = target_sty_user[24*(LSTM_SEQUENCE_LENGTH + DELAY_DAYS) : 24*(LSTM_SEQUENCE_LENGTH + DELAY_DAYS + TRAIN_DAYS)]
        test_target_sty_user = target_sty_user[24*(LSTM_SEQUENCE_LENGTH + DELAY_DAYS + TRAIN_DAYS):]
        # mv_user(label)
        mv_user = pd.DataFrame(edge_user, columns=['ymd_time','from_dong','to_dong','mv_num'])
        to_dong_grouped_user = mv_user.groupby(['ymd_time', 'to_dong']).sum()['mv_num']
        to_dong_grouped_user = to_dong_grouped_user.reset_index()
        
        target_mv_user = torch.zeros([696, 142])
        for i, ymd_time in tqdm(enumerate(ymd_time_lst)):
            for j, dong in enumerate(dong_lst):
                if not to_dong_grouped_user[(to_dong_grouped_user['ymd_time']==ymd_time) & (to_dong_grouped_user['to_dong']==dong)].empty:
                    target_mv_user[i][j] = to_dong_grouped_user[(to_dong_grouped_user['ymd_time']==ymd_time) & (to_dong_grouped_user['to_dong']==dong)]['mv_num'].item()
        train_target_mv_user = target_mv_user[24*(LSTM_SEQUENCE_LENGTH + DELAY_DAYS) : 24*(LSTM_SEQUENCE_LENGTH + DELAY_DAYS + TRAIN_DAYS)]
        test_target_mv_user = target_mv_user[24*(LSTM_SEQUENCE_LENGTH + DELAY_DAYS + TRAIN_DAYS):]
        
        train_target_mv = train_target_mv_infect / train_target_mv_user
        train_target_sty = train_target_sty_infect / train_target_sty_user
        test_target_mv = test_target_mv_infect / test_target_mv_user
        test_target_sty = test_target_sty_infect / test_target_sty_user
        ...
        with open(os.path.join(args.data_path + 'train_' + data_filename + '_new_idx'), 'wb') as f:
            torch.save(train_dataset, f)
        with open(os.path.join(args.data_path + 'test_' + data_filename + '_new_idx'), 'wb') as f:
            torch.save(test_dataset, f)
    
    else:
        train_dataset = torch.load(args.data_path + 'train_' + data_filename + '_new_idx')
        test_dataset = torch.load(args.data_path + 'test_' + data_filename + '_new_idx')
