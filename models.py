...
class GC_TG_LSTM(nn.Module):
    def __init__(self, args):
        super(GC_TG_LSTM, self).__init__()
        ...
        self.graph_fc_user = nn.Linear(142 * args.graph_conv_feature_dim_user, args.lstm_input_size)
        ...
        self.graph_fc_infect = nn.Linear(142 * args.graph_conv_feature_dim_infect, args.lstm_input_size)
        self.graph_act2_infect = nn.ReLU() #  nn.LeakyReLU(negative_slope=0.1) #
        
        # test fc layer
        self.test_fc_user = nn.Linear(142, args.lstm_input_size)
        self.test_fc_infect = nn.Linear(142, args.lstm_input_size)
        ...
        self.lstm_fc = nn.Linear(args.lstm_hidden_size, args.num_dong)
        self.lstm_act2 = nn.ReLU() # nn.LeakyReLU(negative_slope=0.1) #
        
        # temp signal single day 
        self.temp_single_fc = nn.Linear(33, args.lstm_input_size)
        self.temp_single_act = nn.ReLU() # nn.LeakyReLU(negative_slope=0.1)  # 
        
        # temp signal all
        self.temp_all_fc = nn.Linear(33*24*self.lstm_sequence_length, args.num_dong)
        self.temp_all_act =  nn.ReLU() # nn.LeakyReLU(negative_slope=0.1) #
        self.temp_all_output_fc = nn.Linear(args.num_dong*2, args.num_dong)

        self.all_cat_fc = nn.Linear(24*self.lstm_sequence_length*args.lstm_input_size, args.num_dong)
        self.all_cat_act = nn.ReLU()
        self.all_cat_output_fc = nn.Linear(args.num_dong*3, args.num_dong)
    ...
    def forward(self, x):
        ...
        self.lstm_input_tensor = torch.zeros([1, 24*self.lstm_sequence_length, self.lstm_input_size]).to(self.device)
        
        for i in range(24*self.lstm_sequence_length):
            
            graph_user = self.graph_conv_user1(user_graph_list[i][0].reshape([142,-1]).type(torch.FloatTensor).to(self.device), 
...
#             print(self.graph_fc_user.bias.data)
            ...
            graph_infect = self.graph_conv_infect1(infect_graph_list[i][0].reshape([142,-1]).type(torch.FloatTensor).to(self.device), 
...
#             # test
#             graph_user = self.test_fc_user(user_graph_list[i][0].type(torch.FloatTensor).to(self.device))
#             graph_user = self.graph_act2_user(graph_user).squeeze(0)
#             graph_infect = self.test_fc_infect(infect_graph_list[i][0].type(torch.FloatTensor).to(self.device))
#             graph_infect = self.graph_act2_infect(graph_infect).squeeze(0)
...
#             print('lstm_input : ',lstm_input)
        ...
        h_0 = Variable(torch.randn([self.lstm_num_layers, 24*self.lstm_sequence_length, self.lstm_hidden_size], requires_grad=True)).to(self.device)
        c_0 = Variable(torch.randn([self.lstm_num_layers, 24*self.lstm_sequence_length, self.lstm_hidden_size], requires_grad=True)).to(self.device)
